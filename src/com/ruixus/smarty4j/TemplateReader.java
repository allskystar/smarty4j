 ...
 * 与java.io.PushbackReader一样能够将曾经取出的行， 使用回滚的方式将数据重新放回缓冲区中。
 * 
...
public class TemplateReader extends Reader {
	...
	public void addMessage(Level level, String text, int lineNumber, int start, int end) {
		...
		messages.add(new ParseMessage(level, priorInName == null ? text : text + "(" + priorInName
		    + ":" + priorIn.getLineNumber() + ")", lineNumber, start, end));
	}
